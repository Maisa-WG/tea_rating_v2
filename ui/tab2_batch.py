...
def _batch_score_reviews(reviews: List[str], embedder, client, client_d, model_id, r_num, c_num):
    ...
    # 调试信息
    print(f"[DEBUG] 开始批量评分，共 {len(reviews)} 条茶评")
    print(f"[DEBUG] embedder 类型: {type(embedder)}")
    print(f"[DEBUG] client 类型: {type(client)}")
    print(f"[DEBUG] client_d 类型: {type(client_d)}")
    print(f"[DEBUG] model_id: {model_id}")
    print(f"[DEBUG] 知识库数据量: {len(kb[1]) if kb and kb[1] else 0}")
    print(f"[DEBUG] 基础判例数: {len(basic_cases) if basic_cases else 0}")
    print(f"[DEBUG] 进阶判例数: {len(supp_cases[1]) if supp_cases and supp_cases[1] else 0}")

    # 检查必要参数
    if not embedder or not client:
        st.error("❌ 系统未正确初始化，请检查侧边栏配��")
        return
    ...
    for i, review in enumerate(reviews):
        ...
        try:
            ...
            with st.spinner(f"🍵 正在处理第 {i+1}/{total_reviews} 条..."):
                ...
                print(f"[DEBUG] 第 {i+1} 条准备评分，原文长度: {len(review)}")

                # 调用评分逻辑
                print(f"[DEBUG] 开始调用 run_scoring for 第 {i+1} 条")
                ...
                print(f"[DEBUG] 第 {i+1} 条 run_scoring 返回, scores is None: {scores is None}")
                ...
                if scores is None:
                    ...
                    if isinstance(scores, dict) and 'scores' in scores:
                        ...
                        # 如果已经有 master_comment，直接使用
                        actual_scores = scores

                    print(f"[DEBUG] 第 {i+1} 条 scores 类型: {type(scores)}")
                    print(f"[DEBUG] 第 {i+1} 条 actual_scores 类型: {type(actual_scores)}")

                    try:
                        master_comment = _generate_master_comment(actual_scores, review_clean)
                    except Exception as e:
                        print(f"[ERROR] 第 {i+1} 条生成宗师总评失败: {e}")
                        master_comment = "暂无总评"
...
def _generate_master_comment(scores, user_input):
    ...
    try:
        ...
        for factor, data in actual_scores.items():
        ...
        # 智能截断
        def truncate_chinese(text, max_length=100):
        ...
        # 生成总评
        comment = f"此茶{truncate_chinese(user_input, 100)}。"
        ...
        # 添加特色描述
        high_scores = [k.split(' ')[0] for k, v in actual_scores.items()
        ...
        print(f"[ERROR] 生成宗师总评失败: {e}")
        return f"评分已完成（总评生成失败）"
